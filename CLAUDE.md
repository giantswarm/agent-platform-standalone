...
`.nancy-ignore.generated`), regenerated by the org's align-files workflow. A
change to what these files do is made in devctl's templates, never here.
Repo-specific configuration goes next to them: `Makefile.custom.mk`,
`renovate-custom.json5`, `.circleci/custom.yml`.
...
commits (`cliff.toml`, the devctl template's rule): a docs-only PR cuts no
release and is listed in no release notes; its change ships with the next
release.
