...
Output of devctl (the `zz_generated.*` files, `cliff.toml`, the root
`Makefile`, `Makefile.gen.app.mk`, `renovate.json5`, `.circleci/config.yml`,
`.nancy-ignore.generated`), regenerated by the org's align-files workflow.
Repo-specific configuration goes next to them: `Makefile.custom.mk`,
`renovate-custom.json5`, `.circleci/custom.yml`. `cliff.toml` has no side
file; its one repo-specific line (the `docs` skip, see "Commits and
releases") is guarded by `make verify-release-config` and is restored by
hand when an align-files PR regenerates the file.
...
except `docs` and `style` cuts a release through the auto-release workflow and
...
(`fix`, `chore`, `ci`, ...) the patch. git-cliff skips `docs` and `style`
commits (`cliff.toml`): a docs-only PR cuts no release and is listed in no
